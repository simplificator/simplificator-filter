...
class OrdersWithFilters2 < ActiveRecord::Base
  ...
  context "Filterbale" do
    ...
    context "filter_definition" do
      ...
      should "have a filter definition" do
        assert_instance_of Filterable::FilterDefinition, OrdersWithFilters2.filter_definition
      end
    ...
    context "filter_by" do
      ...
      should "skip filter scope 2" do
        carpets = OrdersWithFilters2.filter_by(nil).cheap
        assert_equal 1, carpets.size
      end

      should "find all carpets" do
        carpets = OrdersWithFilters2.filter_by(:fuzzy_name => 'carpet')
        assert_equal 2, carpets.size
      end

      should "find cheaper carpert" do
        carpets = OrdersWithFilters2.filter_by(:fuzzy_name => 'carpet').cheap
        assert_equal 1, carpets.size
        assert_equal 'magic carpet', carpets.first.product_name
      end

      should "find cheaper carpert too" do
        carpets = OrdersWithFilters2.cheap.filter_by(:fuzzy_name => 'carpet')
        assert_equal 1, carpets.size
        assert_equal 'magic carpet', carpets.first.product_name
      end

      should "find expensiver carpet" do
        carpets = OrdersWithFilters2.filter_by(:fuzzy_name => 'carpet', :price_range => '15 - 45')
        assert_equal 1, carpets.size
        assert_equal 'red carpet', carpets.first.product_name
      end

      should "find red carpet" do
        carpets = OrdersWithFilters2.filter_by(:fuzzy_name => 'carpet', :price_range => '15 - 45', :purchased_at => 1.day.ago.to_date)
        assert_equal 1, carpets.size
        assert_equal 'red carpet', carpets.first.product_name
      end

      should "find water bottle" do
        orders = OrdersWithFilters2.filter_by(:price_range => '15 - 45').all(:conditions => {:product_name => 'water bottle'})
        assert_equal 1, orders.size
        assert_equal 'water bottle', orders.first.product_name
      end

      should "accept blank for fuzzy name" do
        orders = OrdersWithFilters2.filter_by(:fuzzy_name => '')
